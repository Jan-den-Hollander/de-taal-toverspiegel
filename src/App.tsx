...
interface Message {
  role: 'user' | 'model';
  nl: string;   // Nederlands (was: de)
  it: string;   // Italiaanse vertaling
  ph?: string;
  score?: number;
  heard?: string;
}
...
export default function App() {
  ...
  const saveCustomKey = (key: string) => {
    ...
    setStatus('Chiave API salvata! · API-sleutel opgeslagen!');
  };
  ...
  const toggleCam = async () => {
    if (isCamOn) {
      ...
      setIsCamOn(false);
      setStatus('Specchio disattivato · Spiegel uitgeschakeld');
    } else {
      try {
  ...
  // TTS: spreek de Nederlandse zin uit
  const speakIt = async (text: string) => {
    if (!text) return;
    setIsSpeaking(true);
    setStatus('Lo specchio parla... · De spiegel spreekt...');
    try {
      const aiInstance = getAI();
      const response = await aiInstance.models.generateContent({
        model: "gemini-2.5-flash-preview-tts",
        contents: [{ parts: [{ text: `Say clearly in Dutch: ${text}` }] }],
      ...
      if (base64Audio) {
        ...
        source.onended = () => { setIsSpeaking(false); setStatus('Premi 🎤 per rispondere · Druk 🎤 om te antwoorden'); };
        source.start();
      } else {
        throw new Error("No audio data received");
      }
    } catch (err) {
      // Terugval: browser TTS met Nederlandse stem
      const utterance = new SpeechSynthesisUtterance(text);
      utterance.lang = 'nl-NL'; utterance.rate = 0.85;
      utterance.onend = () => { setIsSpeaking(false); setStatus('Voce del browser (fallback)'); };
      window.speechSynthesis.speak(utterance);
    }
  };

  // Spraakherkenning: luistert naar Nederlands
  const startRecording = () => {
    try {
      ...
      recognitionRef.current.lang = 'nl-NL'; // Nederlands luisteren
      recognitionRef.current.continuous = false;
      recognitionRef.current.interimResults = false;
      recognitionRef.current.onstart = () => { setIsRecording(true); setStatus('Ascolto... · Ik luister...'); };
  ...
  const processHeard = async (heard: string) => {
    ...
    let currentScore = 0;
    if (lastModelMsg) {
      const similarity = calculateSimilarity(lastModelMsg.nl, heard);
      if (similarity > 0.7) currentScore = 2; else if (similarity > 0.4) currentScore = 1;
      setScore(prev => prev + currentScore);
    }
    const userMsg: Message = { role: 'user', nl: heard, it: '', heard: heard, score: currentScore };
  ...
  const generateAIResponse = async (history: Message[]) => {
    setIsThinking(true);
    setStatus('Lo specchio pensa... · De spiegel denkt na...');

    // Systeem-prompt: Nederlands oefenen voor Italianen
    const systemPrompt = `Sei un simpatico partner di conversazione in olandese — come uno specchio magico che parla.
    Livello: ${level}. Argomento attuale: ${topic}.
    REGOLE: UNA frase breve in olandese per turno (max 12 parole). Termina sempre con una domanda. 
    RISPONDI SOLO con JSON valido: {"nl":"frase in olandese","it":"traduzione italiana","ph":"fonetica olandese"}`;
    ...
    try {
      ...
      const response = await aiInstance.models.generateContent({
        ...
        contents: contents.length > 0 ? contents : [{ role: 'user', parts: [{ text: 'Inizia la conversazione in olandese.' }] }],
        config: { systemInstruction: systemPrompt, responseMimeType: "application/json" },
      });
      const data = JSON.parse(response.text || "{}");
      const aiMsg: Message = { role: 'model', nl: data.nl || "Hallo!", it: data.it || "Ciao!", ph: data.ph || "" };
      setMessages(prev => [...prev, aiMsg]);
      setIsThinking(false);
      speakIt(aiMsg.nl);
    } catch (err) {
      setIsThinking(false);
      setStatus('Ops, lo specchio è appannato. Riprova.');
    }
  };
  ...
  const downloadTranscript = () => {
    ...
    const a = document.createElement('a'); a.href = url; a.download = `conversazione_nl.txt`;
      ...
      <div className="flex flex-col max-w-md mx-auto w-full px-4 pt-4 relative z-10">
        ...
        <header className="text-center pb-4">
          ...
          <p className="text-[0.6rem] tracking-[0.2em] uppercase text-[#c9a84c]/50 mt-1">
            Il tuo partner olandese · Jouw Nederlandse gesprekspartner
          </p>
        </header>
        ...
        <div className="relative mx-auto w-full max-w-[200px] aspect-[3/4] mb-5">
          ...
          >
            ...
            <span className="text-[0.45rem] opacity-60">{isCamOn ? 'Spiegel uit' : 'Spiegel aan'}</span>
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 mb-4">
          <div className="space-y-1">
            <label className="text-[0.55rem] uppercase tracking-widest text-[#c9a84c]/50 ml-1 flex items-center gap-1">
              <Settings size={8} /> Livello · Niveau
            </label>
            <select
          ...
          <div className="space-y-1">
            <label className="text-[0.55rem] uppercase tracking-widest text-[#c9a84c]/50 ml-1 flex items-center gap-1">
              <MessageSquare size={8} /> Argomento · Onderwerp
            </label>
            <select
              value={topic} onChange={(e) => setTopic(e.target.value)}
              className="w-full bg-[#c9a84c]/5 border border-[#c9a84c]/20 rounded-lg px-2 py-2 text-[0.7rem] outline-none text-[#e8c97a]"
            >
              <option value="vita quotidiana">Vita quotidiana · Dagelijks leven</option>
              <option value="ristorante">Ristorante · Restaurant</option>
              <option value="viaggi">Viaggi · Reizen</option>
              <option value="famiglia">Famiglia · Familie</option>
              <option value="lavoro">Lavoro · Werk</option>
            </select>
          </div>
        </div>
        ...
        <div className="flex items-center justify-center gap-6 mb-2">
          <div className="flex flex-col items-center gap-1">
            <button
              type="button" onClick={() => messages.length > 0 && speakIt(messages[messages.length-1].nl)}
            ...
            <span className="text-[0.5rem] uppercase tracking-widest text-[#c9a84c]/60 text-center leading-tight">
              Riascolta<br/><span className="text-[#c9a84c]/40">Herhaal</span>
            </span>
          </div>
          ...
          <div className="flex flex-col items-center gap-1">
            ...
            <span className={`text-[0.55rem] uppercase tracking-widest font-bold text-center leading-tight ${isRecording ? 'text-red-500' : 'text-[#c9a84c]'}`}>
              {isRecording
                ? <>Ascolto...<br/><span className="opacity-60">Ik luister</span></>
                : <>Rispondi<br/><span className="opacity-60">Antwoord</span></>}
            </span>
          </div>
          ...
          <div className="flex flex-col items-center gap-1">
            ...
            <span className="text-[0.5rem] uppercase tracking-widest text-[#c9a84c]/60 text-center leading-tight">
              Salta<br/><span className="text-[#c9a84c]/40">Overslaan</span>
            </span>
          </div>
        </div>
          ...
          {messages.map((msg, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
              <div className={`max-w-[90%] px-3 py-2 rounded-xl text-[0.8rem] leading-relaxed ${msg.role === 'user' ? 'bg-white/5 border border-white/10 rounded-br-none italic text-white/80' : 'bg-gradient-to-br from-[#c9a84c]/10 to-[#c9a84c]/5 border border-[#c9a84c]/20 rounded-bl-none'}`}>
                {msg.role === 'model' ? (
                  <>
                    <span className="font-serif italic text-base text-[#e8c97a] block mb-0.5">{msg.nl}</span>
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between border-b border-[#c9a84c]/10 pb-3">
            <div className="flex items-center gap-1.5 text-[#c9a84c]/60 text-[0.6rem] uppercase tracking-widest">
              <Trophy size={12} /> Punteggio · Score
            </div>
          ...
          >
            ...
            <span className="text-[0.55rem] opacity-60">Nieuw gesprek</span>
          </button>
          ...
          <div className="flex gap-2">
            ...
            >
              ...
              <span className="text-[0.45rem] opacity-60">Opslaan</span>
            </button>
            <button
        ...
        {showKeyModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/80 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-[#12122a] border border-[#c9a84c]/30 p-6 rounded-2xl w-full max-w-xs shadow-2xl">
              <h2 className="font-serif text-xl text-[#e8c97a] mb-2 text-center">API Key · Chiave API</h2>
              <p className="text-[0.6rem] text-[#c9a84c]/50 text-center mb-3">aistudio.google.com</p>
